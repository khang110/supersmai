    ...
    <View style={styles.panelContainer}>
      ...
      <View style={{ height: layoutHeight, overflow: "hidden" }}>
        {dataa.subCategory.map((subitem, key) => (
          ...
          >
            <Text style={{ fontSize: fontSize.fontsize_5 }}>
              {subitem.name}
            </Text>
            <View style={styles.iconStyle}>
              {subitem.checked ? (
...
function ModalFilterCate(props) {
  const CONTENT = [
    {
      expanded: false,
    ...
    <Modal visible={props.modalVisible}>
      <View style={styles.container}>
        <View style={styles.top}>
          ...
          >
            <Text style={{ color: "red", fontSize: fontSize.fontsize_4 }}>
              Bỏ chọn
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={{ margin: "4%" }}>
          <ButtonConfirm title="Tìm theo" onPress={() => filterButton()} />
        </View>
      </View>
    </Modal>
  );
