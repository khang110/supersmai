...
const News = (props) => {
  ...
  const renderStatus = () => {
    if (data.isStatus == "null") {
      return (
            <TouchableOpacity style={styles.btnGive} onPress={onPress}>
              <Text style={{color: color.main_color}}>Gửi tặng</Text>
            </TouchableOpacity>
      )
    }
      ...
      <View style={styles.container}>
        <View style={styles.wrapInfor}>
          ...
          <View style={styles.wrapName}>
            <Text style={styles.name}>{data.usersender.FullName}</Text>
            <Text style={styles.cate}>
              🕑 {calRealTime.CalRealTime(data.usersender.createdAt)}
            </Text>
          </View>
        </View>
        <Text
          style={{ color: color.black, fontSize: fontSize.fontsize_5 }}
          numberOfLines={2}
        >
          {data.note}
        </Text>
        ...
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <Text style={styles.time}>
            {"  "}
            {formatAddress.formatAddress(data.SenderAddress)}
          </Text>
        </View>
        {renderStatus()}
        </View>
